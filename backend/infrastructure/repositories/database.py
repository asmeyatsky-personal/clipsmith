...
engine = create_engine(
    DATABASE_URL, echo=False, connect_args=connect_args, **pool_settings
)
    ...
    with Session(engine) as session:
        ...
        for category, names in template_categories.items():
            for i, name in enumerate(names):
                for style_idx, style in enumerate(styles[:4]):
                    template = AITemplateDB(
                        ...
                        project_data=json.dumps(
                            {"tracks": [], "duration": 30, "style": style}
                        ),
                        is_premium=style_idx >= 2,
                        price=4.99 if style_idx >= 2 else 0.0,
                        tags=json.dumps(
                            [category, style, name.lower().replace(" ", "-")]
                        ),
                    )
        ...
        for name, category, settings_dict in presets:
            preset = ColorGradingPresetDB(
                name=name,
                description=f"Professional {name} color grading preset",
                category=category,
        ...
        for category, effects in effect_categories.items():
            for effect_name in effects:
                is_ar = category == "ar"
                is_premium = category in ("ar", "transition") or effect_name.startswith(
                    "3d_"
                )
